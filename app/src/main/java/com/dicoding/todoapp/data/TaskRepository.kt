...
class TaskRepository(private val tasksDao: TaskDao) {
    ...
    fun getTasks(filter: TasksFilterType): LiveData<PagedList<Task>> {
        ...
        val pagingConfig = PagedList.Config.Builder().apply {
            setEnablePlaceholders(PLACEHOLDERS)
            setPageSize(PAGE_SIZE)
        }.build()
        val pagingSource = tasksDao.getTasks(filterQuery)
        return LivePagedListBuilder(pagingSource, pagingConfig).build()
    }
